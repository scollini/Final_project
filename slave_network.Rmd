...
##Summary Statistics
```{r}
neighbors(slave_graph, 1)
#list.graph.attributes(slave_graph)
vcount(slave_graph)
ecount(slave_graph)
neighborhood(slave_graph, 1, nodes=V(slave_graph))
graph.neighborhood(slave_graph, 1, nodes=V(slave_graph))
...
```{r}
induced.subgraph(slave_graph, which(membership(comm) %in% c(4,6,10))) %>% plot()
set.seed(5)
